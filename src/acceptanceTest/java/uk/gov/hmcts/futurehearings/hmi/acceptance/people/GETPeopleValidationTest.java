...
class GETPeopleValidationTest extends PeopleValidationTest {
    ...
    private static final String UPDATED_SINCE_INVALID_DATE_FORMAT = "Please supply ‘updated_since’ "
            + "date in iso8601 form" ;
    ...
    @ParameterizedTest(name = "Test updated_since param with valid ISO 8601 date format (YYYY-mm-dd)  - "
            + "Param : {0} --> {1}")
    ...
    void test_updated_since_query_param_valid_value(final String updatedSinceKey, final String updatedSinceValue)
            throws Exception {
    ...
    void test_page_without_mandatory_updated_since_queryparam(final String pageKey, final String pageValue)
            throws Exception {
    ...
    void test_all_query_param_with_extra_param(final String paramKey1, final String paramVal1,
        ...
        this.setUrlParams(buildQueryParams(paramKey1, paramVal1, paramKey2, paramVal2, paramKey3,
                paramVal3, paramKey4, paramVal4));
        commonDelegate.test_expected_response_for_supplied_header(
                getAuthorizationToken(),
                getRelativeUrl(), getInputPayloadFileName(),
                createStandardPayloadHeader(),
