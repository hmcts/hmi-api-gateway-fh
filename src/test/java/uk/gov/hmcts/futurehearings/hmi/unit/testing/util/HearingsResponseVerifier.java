...
public final class HearingsResponseVerifier {
    ...
    public static void thenValidateResponseForCreate(Response response) {
        try {
            assertEquals(202, response.getStatusCode(), RESPONSE_CODE_VALIDATION);
    ...
    public static void thenValidateResponseForAdditionalParam(Response response) {
        try {
            ...
            assertEquals(400, response.getStatusCode(), STATUS_CODE_VALIDATION);
            getObjStep().pass("Got the expected status code: 400");
            assertEquals("Invalid query parameter/s in the request URL.", responseMap.get(MESSAGE),
                    STATUS_CODE_MESSAGE_VALIDATION);
            getObjStep().pass(GOT_EXPECTED_MESSAGE + responseMap.get(MESSAGE));
        } catch (AssertionError e) {
            getObjStep().fail(EXCEPTION_IN + e.getMessage());
    ...
    private HearingsResponseVerifier() {
    }
}
